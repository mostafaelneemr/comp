      ...
      <div class="card-header row gutters-5">
            ...
            <div class="col-md-3 ml-auto">
                ...
                <select class="form-control aiz-selectpicker" data-minimum-results-for-search="Infinity"
                    id="cancel_order">
                    <option value="0" @if ($cancel_order == 0) selected @endif>{{ translate('Working') }}</option>
                    <option value="1" @if ($cancel_order == 1) selected @endif>{{ translate('Canceled') }}</option>
                </select>
            </div>
            <div class="col-md-3 ml-auto">
        ...
        <div class="row gutters-5">
			<div class="col text-center text-md-left" style="padding: 2em;"> 
              <address>
                            ...
                            <tr>
                                ...
                                <td>
                                    @if ($orderDetail->product != null)
                                      <a href="{{ route('product', $orderDetail->product->{'slug_' .locale() }) }}" target="_blank"><img height="50px" src="{{ uploaded_asset($orderDetail->product->thumbnail_img) }}"></a>
                                    @else
                                ...
                                <td>
                                    @if ($orderDetail->product != null)
                                      <strong><a href="{{ route('product', $orderDetail->product->{'slug_' .locale() }) }}" target="_blank" class="text-muted">{{ $orderDetail->product->{'name_' . locale()} }}</a></strong>
                                ...
                                <td>
                                    ...
                                    @elseif ($orderDetail->shipping_type == 'pickup_point')
                                      @if ($orderDetail->pickup_point != null)
                                        {{ $orderDetail->pickup_point->{'name_' . locale()} }} ({{ translate('Pickup Point') }})
                                      @else
