                        ...
                        <span class="btn btn-icon p-1">
                            <span class=" position-relative d-inline-block">
                                ...
                                @if ($orders > 0 || $sellers > 0)
                                    <span
                                        class="badge badge-dot badge-circle badge-primary position-absolute absolute-top-right"></span>
                                @endif
                            </span>
                        </span>
                    </a>
                        ...
                        <ul class="list-group c-scrollbar-light overflow-auto" style="max-height:300px;">
                            ...
                            @if ($orders > 0)
                                <li class="list-group-item">
                                    <a href="{{ route('orders.index.admin') }}" class="text-reset">
                ...
                <div class="align-items-stretch d-flex dropdown " id="lang-change">
                    <a class="dropdown-toggle no-arrow" data-toggle="dropdown" href="javascript:void(0);" role="button"
                        ...
                        <span class="btn btn-icon">
                            <img src="{{ static_asset('img/flags/' . $locale . '.png') }}" height="11">
                        </span>
                    </a>
                    <ul class="dropdown-menu dropdown-menu-right dropdown-menu-animated dropdown-menu-xs">
                        ...
                        @foreach (\App\Language::all() as $key => $language)
                            <li>
                                <a href="{{ route('language.change', ['lang' => $language->code]) }}" data-flag="{{ $language->code }}"
                                    class="dropdown-item @if ($locale==$language->code) active @endif">
                                    <img src="{{ static_asset('img/flags/' . $language->code . '.png') }}" class="mr-2">
                                    <span class="language">{{ $language->{'name_' . locale()} }}</span>
                                </a>
                            </li>
                        @endforeach
                    </ul>
                        ...
                        <span class="d-flex align-items-center">
                            <span class="avatar avatar-sm mr-md-2">
                                <img src="{{ uploaded_asset(Auth::user()->avatar_original) }}"
                                    onerror="this.onerror=null;this.src='{{ static_asset('img/avatar-place.png') }}';">
                            </span>
