    ...
    <div class="row no-gutters cols-xs-space cols-sm-space cols-md-space">
        <div class="col-lg-6">
            <div class="product-gal sticky-top d-flex flex-row-reverse">
                @if(is_array(explode(',',$product->photos)) && count(explode(',',$product->photos)) > 0)
                    <div class="product-gal-img">
                        <img src="{{ my_asset('frontend/images/placeholder.jpg') }}" class="xzoom img-fluid lazyload"
                             ...
                             data-src="{{ uploaded_asset(explode(',',$product->photos)[0]) }}"
                             xoriginal="{{ uploaded_asset(explode(',',$product->photos)[0]) }}"/>
                    </div>
                            ...
                            @foreach (explode(',',$product->photos) as $key => $photo)
                                <a href="{{ uploaded_asset($photo) }}">
                                    <img src="{{ my_asset('frontend/images/placeholder.jpg') }}"
                                         ...
                                         @if($key == 0) xpreview="{{ uploaded_asset($photo) }}" @endif>
                                </a>
                            @endforeach
                        </div>
                    </div>
                        ...
                        @if ($product->choice_options != null)
                            @foreach (json_decode($product->choice_options) as $key => $choice)
                                ...
                                <div class="row no-gutters">
                                    <div class="col-2">
                                        <div
                                            class="product-description-label mt-2 ">{{ \App\Attribute::select(['*','name_'.locale().' as name'])->find($choice->attribute_id)->name }}
                                            :
                                        </div>
                                    </div>
                                    <div class="col-10">
