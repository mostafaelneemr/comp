...
class BusinessSettingsController extends Controller
{
    ...
    public function mobilAppSettings(Request $request)
    {
        return view('business_settings.mobil_app_settings');
    }
    ...
    public function google_tags(Request $request)
    {
        // CoreComponentRepository::instantiateShopRepository();
        return view('business_settings.google_tags');
    }
    ...
    public function other_configration(Request $request)
    {
        return view('business_settings.other_configration');
    }
    ...
    public function mobil_app_settings_update(Request $request)
    {
        foreach ($request->types as $key => $type) {
            $saveOne = BusinessSetting::where('type', $type)->first();
            $saveOne->value = $request[$type];
            $saveOne->save();
        }
        $business_settings = BusinessSetting::where('type', 'mobile_app')->first();
        if ($business_settings != null) {
    ...
    public function other_configration_update(Request $request)
    {
        foreach ($request->types as $key => $type) {
            $this->overWriteEnvFile($type, $request[$type]);
        }
        flash(translate("Configration updated successfully"))->success();
        return back();
    }
    ...
    public function google_tags_update(Request $request)
    {
        foreach ($request->types as $key => $type) {
            $saveOne = BusinessSetting::where('type', $type)->first();
            $saveOne->value = $request[$type];
        ...
        $business_settings = BusinessSetting::where('type', 'google_tag')->first();
        if ($business_settings != null) {
            if ($request->has('google_tag')) {
                $business_settings->value = 1;
                $business_settings->save();
            } else {
                $business_settings->value = 0;
    ...
    public function updateActivationSettings(Request $request)
    {
        ...
        if ($request->type == "enable_cache") {
            $business_settings = BusinessSetting::where('type', $request->type)->first();
            if ($business_settings->value == 1) {
                config(['cache.default' => 'file']);
            } else {
                config(['cache.default' => 'none']);
            }
        }
        return '1';
    }
    ...
    {
        $business_settings = BusinessSetting::where('type', $request->type)->first();
        if ($request->type == "free_shipping_by_city") {
            $business_settings->value = implode(',', $request->free_shipping_by_city);
        } else {
    ...
    public function clearCache()
    {
        ...
        flash(translate("All Caches Are Cleared"))->success();
        return back();
    }
