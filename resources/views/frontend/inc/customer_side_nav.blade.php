    ...
    <div class="widget mb-0">
        <div class="widget-profile-box text-center p-3">
            @if (Auth::user()->avatar_original != null)
                <div class="image" style="background-image:url('{{ my_asset(Auth::user()->avatar_original) }}')">
                </div>
            @else
            ...
            <ul class="categories categories--style-3">
                ...
                <li>
                    <a href="{{ route('purchase_history.index') }}"
                        ...
                        <span class="category-name">
                            {{ translate('Purchase History') }} @if ($delivery_viewed > 0 || $payment_status_viewed > 0)<span class="ml-2"
                                    style="color:green"><strong>({{ translate('New Notifications') }})</strong></span>
                            @endif
                        </span>
                    </a>
                </li>
                <li>
                    <a href="{{ route('saved_cards') }}"
                        class="{{ areActiveRoutesHome(['saved_cards']) }}">
                        <i class="la la-file-text"></i>
                        <span class="category-name">
                            {{ translate('Saved Cards') }}</span>
                        </span>
                    </a>
                </li>
                <li>
                ...
                <li>
                    <a href="{{ route('support_ticket.index') }}"
                        ...
                        <span class="category-name">
                            {{ translate('Support Ticket') }} @if ($support_ticket > 0)<span class="ml-2"
                                    style="color:green"><strong>({{ $support_ticket }}
                                        {{ translate('New') }})</strong></span></span>@endif
                        </span>
                    </a>
                </li>
                @if (\App\Addon::where('unique_identifier', 'affiliate_system')->first() != null && \App\Addon::where('unique_identifier', 'affiliate_system')->first()->activated && Auth::user()->affiliate_user != null && Auth::user()->affiliate_user->status)
                    <li class="side-nav-item">
                        <a href="javascript:void(0);"
                            class="side-nav-link {{ areActiveRoutes(['affiliate.user.index', 'affiliate.payment_settings']) }}">
                            <i class="la la-dollar"></i>
                            <span class="side-nav-text">{{ translate('Affiliate') }}</span>
                            <span class="side-nav-arrow"></span>
                        </a>
                        <ul class="side-nav-list level-2">
                            <li class="side-nav-item">
                                <a href="{{ route('affiliate.user.index') }}" class="side-nav-link">
                                    <span class="side-nav-text">{{ translate('Affiliate System') }}</span>
                                </a>
                            </li>
                            <li class="side-nav-item">
                                <a href="{{ route('affiliate.user.payment_history') }}" class="side-nav-link">
                                    <span class="side-nav-text">{{ translate('Payment History') }}</span>
                                </a>
                            </li>
                            <li class="side-nav-item">
                                <a href="{{ route('affiliate.user.withdraw_request_history') }}"
                                    class="side-nav-link">
                                    <span class="side-nav-text">{{ translate('Withdraw request history') }}</span>
                                </a>
                            </li>
                        </ul>
                    </li>
        ...
        @if (\App\BusinessSetting::where('type', 'vendor_system_activation')->first()->value == 1)
            <div class="widget-seller-btn pt-4">
                <a href="{{ route('shops.create') }}"
                    class="btn btn-anim-primary w-100">{{ translate('Be A Seller') }}</a>
            </div>
        @endif
    </div>
</div>
