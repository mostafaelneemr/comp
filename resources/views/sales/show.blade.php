...
@section('content')
    ...
    if ($order->orderDetails()->first()) {
        $order_has_detail = true;
    } else {
        $order_has_detail = false;
    }
    if ($order_has_detail == true) {
    ...
    <div class="card">
        ...
        <div class="card-header row gutters-5">
            ...
            <div class="col-md-3 ml-auto">
                <label for="cancel_order">{{ translate('Cancel Order') }}</label>
                <select class="form-control aiz-selectpicker" data-minimum-results-for-search="Infinity" id="cancel_order">
                    <option value="0" @if ($cancel_order == 0) selected @endif>{{ translate('Working') }}
                    </option>
                    <option value="1" @if ($cancel_order == 1) selected @endif>{{ translate('Canceled') }}
                    </option>
                </select>
            </div>
            @if ($order_has_detail == true)
                ...
                <div class="col-md-3 ml-auto">
                    ...
                    <select class="form-control aiz-selectpicker" data-minimum-results-for-search="Infinity"
                        ...
                        <option value="on_review" @if ($delivery_status == 'on_review') selected @endif>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            @else
                                <td>
                                    <span
                                        class="badge badge-inline badge-danger">{{ translate('Not complete order') }}</span>
                                </td>
                            @endif
                        ...
                        @if ($order_has_detail == true)
                            <tr>
                                <td class="text-main text-bold">
                                    {{ translate('Seller Order Status') }}
                                </td>
                                @php
                                    $seller_status = $order->orderDetails->first()->seller_status;
                                @endphp
                                <td class="text-right">
                                    ...
                                    @else
                                        <span
                                            class="badge badge-info">{{ ucfirst(str_replace('_', ' ', $seller_status)) }}</span>
                                    @endif
                                </td>
                            </tr>
                        @endif
                            ...
                            <tr>
                                ...
                                <td>
                                    @if ($orderDetail->product != null)
                                        <a href="{{ route('product', $orderDetail->product->{'slug_' . locale()}) }}"
                                ...
                                <td>
                                    @if ($orderDetail->product != null)
                                        <strong><a
                                                href="{{ route('product', $orderDetail->product->{'slug_' . locale()}) }}"
                                                target="_blank">{{ $orderDetail->product->{'name_' . locale()} }}</a></strong>
...
@section('script')
    <script type="text/javascript">
        ...
        $('#cancel_order').on('change', function() {
            var order_id = {{ $order->id }};
            var cancel_order = $('#cancel_order').val();
            $.post('{{ route('orders.cancel_order') }}', {
                _token: '{{ @csrf_token() }}',
                order_id: order_id,
                cancel_order: cancel_order
            }, function(data) {
                AIZ.plugins.notify('success', '{{ translate('Order has been updated') }}');
            });
        });
    </script>
@endsection
