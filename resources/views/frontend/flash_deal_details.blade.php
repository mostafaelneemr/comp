...
@section('content')
    ...
    <div style="background-color:{{ $flash_deal->background_color }}">
        <section class="text-center">
            <img src="{{ uploaded_asset($flash_deal->banner) }}" alt="{{ $flash_deal->{'title_'.locale()} }}" class="img-fit w-100">
        </section>
        <section class="pb-4">
            <div class="container">
                <div class="text-center my-4 text-{{ $flash_deal->text_color }}">
                    <h1 class="h3">{{ $flash_deal->{'title_'.locale()}  }}</h1>
                                    ...
                                    <div class="card-body p-0">
                                        ...
                                        <div class="card-image">
                                            <a href="{{ route('product', $product->{'slug_'.locale()}) }}" class="d-block text-center" >
                                                <img class="img-fit lazy" src="{{ my_asset('frontend/images/placeholder.jpg') }}" data-src="{{ uploaded_asset($product->thumbnail_img) }}" alt="{{  __($product->{'name_'.locale()}) }}">
                                            </a>
                                        </div>
                                        ...
                                        <div class="p-3">
                                            ...
                                            <h2 class="product-title p-0 mt-2">
                                                <a href="{{ route('product', $product->{'slug_'.locale()}) }}" class="text-truncate">{{  __($product->{'name_'.locale()}) }}</a>
                                            </h2>
                                        </div>
                                    </div>
                                </div>
    ...
    @else
        <div style="background-color:{{ $flash_deal->background_color }}">
            <section class="text-center pt-3">
                <div class="container ">
                    <img src="{{ uploaded_asset($flash_deal->banner) }}" alt="{{ $flash_deal->{'title_'.locale()} }}" class="img-fit">
                </div>
            </section>
            <section class="pb-4">
                <div class="container">
                    <div class="text-center text-{{ $flash_deal->text_color }}">
                        <h1 class="h3 my-4">{{ $flash_deal->{'title_'.locale()} }}</h1>
