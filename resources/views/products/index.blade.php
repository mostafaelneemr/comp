...
@section('content')
    ...
    <div class="aiz-titlebar text-left mt-2 mb-3">
        <div class="row align-items-center">
            <div class="col-md-6">
                <h1 class="h3">{{ translate($type . ' Products') }}</h1>
            </div>
            @if ($type != 'Seller')
            ...
            @else
                <div class="col-md-6 text-md-right">
                    <a href="{{ route('products.seller_create') }}" class="btn btn-circle btn-info">
                        <span>{{ translate('Add New Product') }}</span>
                    </a>
                </div>
            @endif
                ...
                <table class="table aiz-table mb-0">
                    <thead>
                        <tr>
                            ...
                            <th data-breakpoints="md">{{ translate('Num of Sale') }}</th>
                        ...
                        @foreach ($products as $key => $product)
                            <tr>
                                ...
                                <td>
                                    ...
                                    <span class="text-muted">{{ $product->{'name_' . locale()} }}</span>

                                </td>
                                <td>
                                    <a href="{{ route('product', $product->{'slug_' . locale()}) }}" target="_blank">
                                ...
                                @endif
                                <td>{{ translate($product->{'country_' . locale()}) }}</td>
    ...
    echo 'checked';
                                ...
                                <td class="text-right">
                                    <a href="javascript:void(0)" title="{{ translate('Copy Deep Link') }}"
                                        ...
                                        data-url="{{ route('product', [$product->id]) }}">
                                        <i class="las la-clipboard mr-2"></i>
                                    </a>
                                    @if ($type == 'Seller')
                                        <a class="btn btn-soft-primary btn-icon btn-circle btn-sm"
                                            href="{{ route('products.seller.edit', encrypt($product->id)) }}"
                                            title="{{ translate('Edit') }}">
                                            <i class="las la-edit"></i>
                                        </a>
                                    @else
                                        <a class="btn btn-soft-primary btn-icon btn-circle btn-sm"
                                            href="{{ route('products.admin.edit', encrypt($product->id)) }}"
                                            title="{{ translate('Edit') }}">
                                            <i class="las la-edit"></i>
                                        </a>
                                    @endif
