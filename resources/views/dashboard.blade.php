            ...
            <div class="col-6">
                <div class="bg-grad-1 text-white rounded-lg mb-4 overflow-hidden">
                    <div class="px-3 pt-3">
                        ...
                        <div class="h3 fw-700 mb-3">{{ \App\Category::where('published',true)->count() }}</div>
                    </div>
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1440 320">
                        <path fill="rgba(255,255,255,0.3)" fill-opacity="1" d="M0,128L34.3,112C68.6,96,137,64,206,96C274.3,128,343,224,411,250.7C480,277,549,235,617,213.3C685.7,192,754,192,823,181.3C891.4,171,960,149,1029,117.3C1097.1,85,1166,43,1234,58.7C1302.9,75,1371,149,1406,186.7L1440,224L1440,320L1405.7,320C1371.4,320,1303,320,1234,320C1165.7,320,1097,320,1029,320C960,320,891,320,823,320C754.3,320,686,320,617,320C548.6,320,480,320,411,320C342.9,320,274,320,206,320C137.1,320,69,320,34,320L0,320Z"></path>
                    </svg>
                    ...
                    <div class="aiz-card-box border border-light rounded shadow-sm hov-shadow-md mb-2 has-transition bg-white">
                        ...
                        <div class="p-md-3 p-2 text-left">
                            ...
                            <h3 class="fw-600 fs-13 text-truncate-2 lh-1-4 mb-0">
                                <a href="{{ route('product', $product->{'slug_'.locale()}) }}" class="d-block text-reset">{{ $product->{'name_' . locale() } }}</a>
                            </h3>
                        </div>
                    </div>
                </div>
...
<script type="text/javascript">
    ...
    AIZ.plugins.chart('#graph-1',{
        ...
        data: {
            labels: [
                @foreach (\App\Category::where('level', 0)->where('published',true)->get() as $key => $category)
            ...
            datasets: [{
                ...
                data: [
                    @foreach (\App\Category::where('level', 0)->where('published',true)->get() as $key => $category)
                        @php
                        $this->category_ids = \App\Utility\CategoryUtility::children_ids($category->id);
                        $this->category_ids[] = $category->id;
                        @endphp
                        {{ \App\Product::join('product_sub_sub_categories', function ($join) {$join->on('products.id', '=', 'product_sub_sub_categories.product_id')->whereIn('product_sub_sub_categories.subsubcategory_id', $this->category_ids);})->sum('products.num_of_sale') }},
                    @endforeach
                ],
                backgroundColor: [
                    @foreach (\App\Category::where('level', 0)->where('published',true)->get() as $key => $category)
                ...
                data: [
                    @foreach (\App\Category::where('level', 0)->where('published',true)->get() as $key => $category)
                        @php
                            ...
                            $products = \App\Product::select(['products.*'])->join('product_sub_sub_categories', function ($join) {$join->on('products.id', '=', 'product_sub_sub_categories.product_id')->whereIn('product_sub_sub_categories.subsubcategory_id', $this->category_ids);})->distinct()->get();
                            $qty = 0;
