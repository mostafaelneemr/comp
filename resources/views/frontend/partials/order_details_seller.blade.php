...
@php
    ...
    $seller_status = $order->orderDetails->where('seller_id', Auth::user()->id)->first()->seller_status;
    ...
    <div class="row mt-5">
        <div class="offset-lg-2 col-lg-4 col-sm-6">
            <div class="form-inline">
                <select class="form-control selectpicker form-control-sm"  data-minimum-results-for-search="Infinity" id="update_seller_status">
                   @if(empty($seller_status))
                        <option>{{translate('Accept Or Reject This Order')}}</option>
                    @endif
                    <option value="accept" @if ($seller_status == 'accept') selected @endif>{{ translate('Accept')}}</option>
                    <option value="reject" @if ($seller_status == 'reject') selected @endif>{{ translate('Reject')}}</option>
                </select>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        @if ($orderDetail->product != null)
                                            <a href="{{ route('product', $orderDetail->product->{'slug_'.locale()}) }}" target="_blank">{{ $orderDetail->product->{'name_'.locale()} }}</a>
                                        @else
...
<script type="text/javascript">
    ...
    $('#update_seller_status').on('change', function(){
        var order_id = {{ $order->id }};
        var status = $('#update_seller_status').val();
        $.post('{{ route('orders.update_seller_status') }}', {_token:'{{ @csrf_token() }}',order_id:order_id,status:status}, function(data){
            $('#order_details').modal('hide');
            //console.log(data);
            showFrontendAlert('success', 'status has been updated');
            location.reload().setTimeOut(500);
        });
    });
</script>
