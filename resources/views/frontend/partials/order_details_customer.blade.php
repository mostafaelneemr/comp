...
<div class="modal-body gry-bg px-3 pt-0">
    ...
    @else
    <br>
    <div class="text-center strong-600">{{ translate('Order has been canceled') }}</div>
    @endif
    ...
    <div class="row">
        <div class="col-lg-9">
            <div class="card mt-4">
                <div class="card-header py-2 px-3 heading-6 strong-600">{{ translate('Order Details') }}
                    @if (($order->orderDetails[0]->delivery_status == 'pending' or $order->orderDetails[0]->delivery_status = 'on_review') && $order->cancel_order == 0)
                        <a href="{{ route('cancel_order_user', $order->id) }}"
                            class="btn btn-styled btn-sm btn-base-1">{{ translate('Cancel Order') }}</a>
                                    ...
                                    <td>
                                        @if ($orderDetail->product != null)
                                            <a href="{{ route('product', $orderDetail->product->{'slug_' . locale()}) }}"
                                                target="_blank">{{ $orderDetail->product->{'name_' . locale()} }}</a>
                                        @else
                ...
                <div class="card-body pb-0">
                    <table class="table details-table">
                        <tbody>
                            ...
                            @if ((isset(json_decode($order->shipping_address)->shipping_date)))
                            <tr>

                                <th>{{ translate('Shipping date') }}</th>
                                <td class="text-right">
                                    <span
                                        class="text-italic">{{ json_decode($order->shipping_address)->shipping_date }}</span>
                                </td>
                            </tr>
                            @endif
