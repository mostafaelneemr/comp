    ...
    @if (count($subsubcategories) > 0)
        ...
        <ul>
            @foreach ($subsubcategories as $key => $subsubcategory)
                <li><a href="{{ route('products.subsubcategory', $subsubcategory->{'slug_' . locale()}) }}">{{ $subsubcategory->name }}</a></li>
            @endforeach
        </ul>
    @endif
</div>
    ...
    @if (count($products) > 0)
        ...
        <ul>
            @foreach ($products as $key => $product)
                <li>
                    <a href="{{ route('product', $product->{'slug_'.locale()}) }}">
                        <div class="d-flex search-product align-items-center">
                            <div class="image" style="background-image:url('{{ uploaded_asset($product->thumbnail_img) }}');">
                            </div>
                ...
                @foreach ($shops as $key => $shop)
                    <li>
                        <a href="{{ route('shop.visit', $shop->slug) }}">
                            <div class="d-flex search-product align-items-center">
                                <div class="image" style="background-image:url('{{ uploaded_asset($shop->logo) }}');">
                                </div>
                                <div class="w-100 overflow--hidden ">
