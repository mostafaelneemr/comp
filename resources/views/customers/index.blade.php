    ...
    <div class="aiz-titlebar text-left mt-2 mb-3">
        <div class="row align-items-center">
            ...
            <div class="col-md-6 text-md-right">
                <a href="{{ action('CustomerController@export') }}" class="btn btn-circle btn-info">
                            ...
                            <tr>
                                ...
                                <td>
                                    @if ($customer->user->banned == 1) <i
                                            class="fa fa-ban text-danger" aria-hidden="true"></i> @endif {{ $customer->user->name }}
                                </td>
                                ...
                                <td>
                                    @if ($customer->user)
                                        <label class="aiz-switch aiz-switch-success mb-0">
                                            <input onchange="update_active(this)" value="{{ $customer->user->id }}"
                                                type="checkbox" <?php if ($customer->user->email_verified_at != null)
                                            {
                                            echo 'checked';
                                            } ?> >
...
@section('script')
    <script type="text/javascript">
        ...
        function update_active(el) {
            ...
            $.post(`{{ route('customers.activeCustomer') }}`, {
                    _token: '{{ csrf_token() }}',
                    id: el.value,
                    status: status
                },
                function(data) {
                    if (data == 1) {
                        AIZ.plugins.notify('success', '{{ translate('Active user updated successfully') }}');
                    } else {
