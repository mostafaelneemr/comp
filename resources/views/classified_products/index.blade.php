...
@section('content')
    ...
    <div class="card">
        <div class="card-header row gutters-5">
            <h3 class="mb-md-0 h6">{{ translate('Classified Products') }}</h3>
        </div>
        <div class="card-body">
            <table class="table aiz-table mb-0">
                ...
                <tbody>
                    @foreach ($products as $key => $product)
                        <tr>
                            <td>{{ $key + 1 }}</td>
                            <td><a href="{{ route('customer.product', $product->slug) }}"
                                    target="_blank">{{ $product->name }}</a></td>

                            <td><img class="h-50px" src="{{ uploaded_asset($product->thumbnail_img) }}" alt="Logo"></td>
                            ...
                            <td class="text-right">
                                <a href="javascript:void(0)" title="{{ translate('Copy Deep Link') }}"
                                    ...
                                    data-url="{{ route('customer.product', $product->id) }}">
