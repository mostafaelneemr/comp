...
@section('content')
    ...
    <div class="aiz-titlebar text-left mt-2 mb-3">
        <div class="row align-items-center">
            <div class="col-md-6">
                <h1 class="h3">{{ translate('Flash Deals') }}</h1>
            </div>
            <div class="col-md-6 text-md-right">
                <a href="{{ route('flash_deals.create') }}" class="btn btn-circle btn-info">
                    <span>{{ translate('Add New Flash Deal Products') }}</span>
                </a>
            </div>
        </div>
    </div>
                    ...
                    <tr>
						...
						<td>
                            <a class="btn btn-primary" target="_blank"
                                    href="{{ url('flash-deal/' . $flash_deal->{'slug_' . locale()}) }}">{{ $flash_deal->{'slug_' . locale()} }}</a>
                            </td>
						<td class="text-right">
                            <a class="btn btn-soft-primary btn-icon btn-circle btn-sm" href="{{ route('flash_deals.edit', encrypt($flash_deal->id)) }}" title="{{ translate('Edit') }}">
