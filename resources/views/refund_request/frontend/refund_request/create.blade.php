                        ...
                        <form class="" action="{{route('refund_request_send', $order_detail->id)}}" method="POST" enctype="multipart/form-data" id="choice_form">
                            ...
                            <div class="form-box bg-white mt-4">
                                <div class="form-box-content p-3">
                                    ...
                                    <div class="row">
                                        <div class="col-md-2">
                                            <label>{{ translate('Refund resone')}} <span class="required-star">*</span></label>
                                        </div>
                                        <div class="col-md-10">
                                            <div class="mb-3">
                                                <select class="form-control mb-3 selectpicker" data-placeholder="{{ translate('Select a resone')}}" id="resone_id" name="resone_id" required>
                                                    @foreach ($resons as $key => $reson)
                                                        <option value="{{ $reson->id }}">{{ $reson->resone }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </div>
