...
@section('content')
    @if ($open && $model_setting->status == 1)
        <div id="myModal" class="modal fade" role="dialog">
            <div class="modal-dialog">
                <!-- Modal content-->
                <div class="modal-content">
                    <div class="modal-header">
                        <h4 class="modal-title">{{ $model_setting->{'name_' . locale()} }}</h4>
                    </div>
                    <div class="modal-body">
                        <p>{{ $model_setting->{'description_' . locale()} }}</p>
                    </div>
                        ...
                        <ul class="categories no-scrollbar">
                            <li class="d-lg-none">
                                <a href="{{ route('categories.all') }}" class="text-truncate">
                                    <img class="cat-image lazy" src="{{ my_asset('frontend/images/placeholder.jpg') }}"
                                        data-src="{{ my_asset('frontend/images/icons/list.png') }}" width="30"
                                        alt="{{ translate('All Category') }}">
                                    <span class="cat-name">{{ translate('All') }} <br>
                                        {{ translate('Categories') }}</span>
                                </a>
                            </li>

                            @foreach (\App\Category::where('level', 0)->where('published', true)->select(['*', 'name_' . locale() . ' as name'])->get()->take(11)
        as $key => $category)
                                ...
                                <li class="category-nav-element" data-id="{{ $category->id }}">
                                    <a href="{{ route('products.category', $category->{'slug_' . locale()}) }}"
                                        class="text-truncate">
                                        @if ($category->icon)
                                            <img class="cat-image" src="{{ uploaded_asset($category->icon) }}" width="30"
                                                alt="{{ $category->name }}">
                                        @else
                                            <img class="cat-image"
                                                src="{{ my_asset('frontend/images/placeholder.jpg') }}" width="30"
                                                alt="{{ $category->name }}">
                                        @endif
                ...
                @php
                    ...
                    $featured_categories = \App\Category::where('featured', 1)
                                ...
                                <div class="slick-carousel" data-slick-arrows="true" data-slick-dots="true"
                                    ...
                                    @foreach (\App\Slider::where('published', 1)->get() as $key => $slider)
                                        <div class="home-slide-item" style="height:275px;">
                                            <a href="{{ $slider->{'link_' . locale()} }}" target="_blank">
                                                <img class="d-block w-100 h-100 lazyload"
                                                    src="{{ my_asset('frontend/images/placeholder-rect.jpg') }}"
                                                    data-src="{{ uploaded_asset($slider->{'photo_web_' . locale()}) }}"
                                                ...
                                                <a href="{{ route('products.category', $category->{'slug_' . locale()}) }}"
                                                    ...
                                                    <div class="img">
                                                        <img src="{{ my_asset('frontend/images/placeholder.jpg') }}"
                                                            ...
                                                            alt="{{ $category->name }}" class="lazy img-fit">
                                                    </div>
                                                </a>
                                            </div>
                                        </li>
                    ...
                    <div class="col-lg-2 d-none d-lg-block">
                        <div class="flash-deal-box bg-white h-100">
                            ...
                            <div class="flash-content c-scrollbar c-height">
                                @foreach (filter_products(\App\Product::where('published', 1)->where('todays_deal', '1'))->select(['*', 'name_' . locale() . ' as name', 'slug_' . locale() . ' as slug'])->get()
        as $key => $product)
                                    @if ($product != null)
                                                ...
                                                <div class="col">
                                                    <div class="price">
                                                        ...
                                                        @if (home_base_price($product->id) != home_discounted_base_price($product->id))
                                                            <del
                                                                class="d-block">{{ home_base_price($product->id) }}</del>
                                                        @endif
                                                    </div>
                                                </div>
                                            </div>
                ...
                <div class="px-2 py-4 p-md-4 bg-white shadow-sm">
                    <div class="section-title-1 clearfix ">
                        ...
                        <ul class="inline-links float-right">
                            <li><a href="{{ route('flash-deal-details', $flash_deal->{'slug_' . locale()}) }}"
                    ...
                    <div class="caorusel-box arrow-round gutters-5">
                        <div class="slick-carousel" data-slick-items="6" data-slick-xl-items="5" data-slick-lg-items="4"
                            ...
                            @foreach ($flash_deal->flash_deal_products as $key => $flash_deal_product)
                                @php
                                    $product = \App\Product::select(['*', 'name_' . locale() . ' as name', 'slug_' . locale() . ' as slug'])->find($flash_deal_product->product_id);
                                @endphp
                                                ...
                                                <div class="p-md-3 p-2">
                                                    ...
                                                    <h2 class="product-title p-0">
                                                        <a href="{{ route('product', $product->slug) }}"
                                                            class=" text-truncate">{{ $product->name }}</a>
                                                    </h2>
    ...
    <div class="mb-4">
        <div class="container">
            <div class="row gutters-10">
                @foreach (\App\Banner::where(['position' => 1, 'published' => 1, 'mobile_web' => 0])->get() as $key => $banner)
                    <div
                        class="col-lg-{{ 12 / count(\App\Banner::where(['position' => 1, 'published' => 1, 'mobile_web' => 0])->get()) }}">
                        <div class="media-banner mb-3 mb-lg-0">
                            <a href="{{ $banner->{'url_' . locale()} }}" target="_blank" class="banner-container">
                                <img src="{{ my_asset('frontend/images/placeholder-rect.jpg') }}"
                                    data-src="{{ uploaded_asset($banner->{'photo_' . locale()}) }}"
                            ...
                            <div class="slick-carousel" data-slick-items="6" data-slick-xl-items="5" data-slick-lg-items="4"
                                ...
                                @foreach ($customer_products as $key => $customer_product)
                                    <div class="product-card-2 card card-product my-2 mx-1 mx-sm-2 shop-cards shop-tech">
                                        <div class="card-body p-0">
                                            <div class="card-image">
                                                <a href="{{ route('customer.product', $customer_product->{'slug_' . locale()}) }}"
                                                    class="d-block">
                                                    <img class="img-fit lazyload mx-auto"
                                                        src="{{ my_asset('frontend/images/placeholder.jpg') }}"
                                                        data-src="{{ uploaded_asset($customer_product->thumbnail_img) }}"
                                                        alt="{{ $customer_product->{'name_' . locale()} }}">
                                                </a>
                                            </div>
                                            ...
                                            <div class="p-sm-3 p-2">
                                                ...
                                                <h2 class="product-title p-0 text-truncate-1">
                                                    <a
                                                        href="{{ route('customer.product', $customer_product->{'slug_' . locale()}) }}">{{ $customer_product->{'name_' . locale()} }}</a>
                                                </h2>
                                                <div>
    ...
    <div class="mb-4">
        <div class="container">
            <div class="row gutters-10">
                @foreach (\App\Banner::where(['position' => 2, 'published' => 1, 'mobile_web' => 0])->get() as $key => $banner)
                    <div
                        class="col-lg-{{ 12 / count(\App\Banner::where(['position' => 2, 'published' => 1, 'mobile_web' => 0])->get()) }}">
                        <div class="media-banner mb-3 mb-lg-0">
                            <a href="{{ $banner->{'url_' . locale()} }}" target="_blank" class="banner-container">
                                <img src="{{ my_asset('frontend/images/placeholder-rect.jpg') }}"
                                    data-src="{{ uploaded_asset($banner->{'photo_' . locale()}) }}"
    ...
    @if (count(
            \App\Category::where('top', 1)->where('published', true)->get(),
        ) != null &&
        count(\App\Brand::where('top', 1)->get()) != null)
        <section class="mb-3">
            <div class="container">
                <div class="row gutters-10">
                    <div class="col-lg-6">
                        ...
                        <div class="row gutters-5">
                            @foreach (\App\Category::where('top', 1)->where('published', true)->select(['*', 'name_' . locale() . ' as name'])->get()
        as $category)
                                    ...
                                    <a href="{{ route('products.category', $category->{'slug_' . locale()}) }}"
                                        ...
                                        <div class="row align-items-center no-gutters">
                                            <div class="col-3 text-center">
                                                <img src="{{ my_asset('frontend/images/placeholder.jpg') }}"
                                                    data-src="{{ uploaded_asset($category->banner) }}"
                                                    alt="{{ $category->name }}" class="img-fluid img lazyload">
                                            </div>
                                            <div class="info col-7">
                                                <div class="name text-truncate pl-3 py-4">{{ $category->name }}</div>
                                            </div>
                    ...
                    <div class="col-lg-6">
                        ...
                        <div class="row gutters-5">
                            @foreach (\App\Brand::where('top', 1)->get() as $brand)
                                <div class="mb-3 col-6">
                                    <a href="{{ route('products.brand', $brand->{'slug_' . locale()}) }}"
                                        class="bg-white border d-block c-base-2 box-2 icon-anim pl-2">
                                        <div class="row align-items-center no-gutters">
                                            <div class="col-3 text-center">
                                                <img src="{{ my_asset('frontend/images/placeholder.jpg') }}"
    ...
    @endif
    @php
    $promotion = App\Models\AppSettings::select('promotion_title_ar', 'promotion_title_en', 'promotion_desc_ar', 'promotion_desc_en', 'promotion_photo', 'promotion_link_ar', 'promotion_link_en', 'promotion_appear')->first();

    @endphp

    <div class="modal custom fade" style="" id="Promotion" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1"
        role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-sm" id="Promotion-dialog" role="document">
            <div class="modal-content">
                <button type="button" class="close absolute-close-btn" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
                <div class="card text-center">
                    <img class="card-img-top" src="{{ uploaded_asset($promotion->promotion_photo) }}"
                        alt="Card image cap">
                    <div class="card-body">
                        <h5 class="card-title">{{ $promotion->{'promotion_title_' . locale()} }}</h5>
                        <p class="card-text">{{ $promotion->{'promotion_desc_' . locale()} }}</p>
                        <a href="{{ $promotion->{'promotion_link_' . locale()} }}"
                            class="btn btn-primary">{{ translate('OK! Lets go') }}</a>
                    </div>
                </div>
            </div>
        </div>
        ...
        $(document).ready(function() {
            $(window).scroll(function() {
                if ($(window).scrollTop() + $(window).height() == $(document).height()) {
                    @if ($promotion->promotion_appear == true)
