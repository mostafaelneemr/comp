...
@section('meta')
    ...
    <meta name="twitter:data1" content="{{ home_price($detailedProduct->id) }}">
    <!-- Twitter Card data -->
    ...
    <meta property="og:url" content="{{ route('product', $detailedProduct->{'slug_' . locale()}) }}">

    <meta property="og:image" content="{{ uploaded_asset($detailedProduct->meta_img) }}">
    <meta property="og:price:amount" content="{{ home_price($detailedProduct->id) }}">
    <meta property="product:price:currency" content="EGP">

    <!-- Schema.org markup for Google+ -->

    <meta itemprop="name" content="{{ $detailedProduct->{'name_' . locale()} }}">

    <meta itemprop="description" content="{{ $detailedProduct->{'description_' . locale()} }}">
@endsection
...
@section('content')
                        ...
                        <div class="container">
                            ...
                            <div class="float-right">
                                <a class="btn btn-styled btn-base-1 btn-icon-left strong-700 hov-bounce hov-shaddow"
                                    href="{{ route('product', [$detailedProduct->id]) }}"
                ...
                <div class="col">
                    <ul class="breadcrumb">
                        ...
                        @if ($crumbArr['category_id'] != null)
                            <li><a
                                    href="{{ route('products.category', \App\Category::find($crumbArr['category_id'])->{'slug_' . locale()}) }}">{{ \App\Category::find($crumbArr['category_id'])->{'name_' . locale()} }}</a>
                            </li>
                        @endif
                        @if ($crumbArr['sub_category_id'] != null)
                            <li><a
                                    href="{{ route('products.category', \App\Category::find($crumbArr['sub_category_id'])->{'slug_' . locale()}) }}">{{ \App\Category::find($crumbArr['sub_category_id'])->{'name_' . locale()} }}</a>
                            </li>
                        @endif
                        @if ($crumbArr['sub_sub_category_id'] != null)
                            <li><a
                                    href="{{ route('products.category', \App\Category::find($crumbArr['sub_sub_category_id'])->{'slug_' . locale()}) }}">{{ \App\Category::find($crumbArr['sub_sub_category_id'])->{'name_' . locale()} }}</a>
                            </li>
                        @endif

                        <li class="active"><a href="#">{{ $detailedProduct->name }}</a></li>
                    </ul>
                </div>
            </div>
        </div>
                    ...
                    <div class="col-lg-6">
                        <div class="product-gal sticky-top d-flex flex-row-reverse">
                            ...
                            <div class="product-gal-img">
                                <img src="{{ my_asset('frontend/images/placeholder.jpg') }}" class="xzoom img-fluid lazy"
                                    alt="{{ $detailedProduct->name }}"
                                    src="{{ my_asset('frontend/images/placeholder.jpg') }}"
                                    data-src="{{ uploaded_asset($photos[0]) }}"
                                    xoriginal="{{ uploaded_asset($photos[0]) }}" />
                            </div>
                                    ...
                                    @foreach ($photos as $key => $photo)
                                        <a href="{{ uploaded_asset($photo) }}">
                                            <img src="{{ my_asset('frontend/images/placeholder.jpg') }}"
                                                alt="{{ $detailedProduct->name }}" class="xzoom-gallery lazy"
                                                src="{{ my_asset('frontend/images/placeholder.jpg') }}" width="80"
                                                data-src="{{ uploaded_asset($photo) }}" @if ($key == 0) xpreview="{{ uploaded_asset($photo) }}" @endif>
                                        </a>
                                    @endforeach
                                </div>
                            </div>
                        ...
                        <div class="product-description-wrapper">
                            ...
                            <div class="row align-items-center my-1">
                                <div class="col-6">
                                    ...
                                    <div class="rating">
                                        ...
                                        <span class="rating-count ml-1">({{ $total }}
                                ...
                                <div class="col-6 text-right">
                                    <ul class="inline-links inline-links--style-1">
                                        ...
                                        @if ($qty > 0)
                                            <li>
                                                <span
                                                    class="badge badge-md badge-pill bg-green">{{ translate('In stock') }}</span>
                                            </li>
                                        @else
                                            <li>
                                                <span
                                                    class="badge badge-md badge-pill bg-red">{{ translate('Out of stock') }}</span>
                                            </li>
                                        @endif
                                    </ul>
                                </div>
                            ...
                            <div class="row align-items-center">
                                <div class="sold-by col-auto">
                                    ...
                                    @if ($detailedProduct->added_by == 'seller' && \App\BusinessSetting::where('type', 'vendor_system_activation')->first()->value == 1)
                                        <a
                                            href="{{ route('shop.visit', $detailedProduct->user->shop->slug) }}">{{ $detailedProduct->user->shop->{'name_' . locale()} }}</a>
                                    @else
                                ...
                                @if ($detailedProduct->brand != null)
                                    <div class="col-auto">
                                        <img src="{{ uploaded_asset($detailedProduct->brand->logo) }}"
                                            alt="{{ $detailedProduct->brand->name }}" height="30">
                                    </div>
                                @endif
                            </div>

                            <hr>
                            @isset($detailedProduct->{'country_' . locale()})
                                <div class="row align-items-center">
                                    <div class="sold-by col-auto">
                                        <small class="mr-2">{{ translate('Country') }}: </small>
                                        <small class="mr-2"> {{ $detailedProduct->{'country_' . locale()} }}
                                        </small>
                                    </div>
                            ...
                            @if (home_price($detailedProduct->id) != home_discounted_price($detailedProduct->id))
                                ...
                                <div class="row no-gutters mt-3">
                                    <div class="col-2">
                                        <div class="product-description-label mt-1">{{ translate('Discount Price') }}:
                                        </div>
                                    </div>
                                    <div class="col-10">
                            ...
                            <form id="option-choice-form">
                                ...
                                @if ($detailedProduct->choice_options != null)
                                    @foreach (json_decode($detailedProduct->choice_options) as $key => $choice)
                                        ...
                                        <div class="row no-gutters" @if (sizeof($choice->values) == 1) style="display:none"  @endif>
                                            <div class="col-2">
                                                <div class="product-description-label mt-2 ">
                                                    {{ \App\Attribute::select(['*', 'name_' . locale() . ' as name'])->find($choice->attribute_id)->name }}:
                                                </div>
                                            </div>
                                            <div class="col-10">
                                                <ul
                                ...
                                @if (count(json_decode($detailedProduct->colors)) > 0)
                                    <div class="row no-gutters" @if (count(json_decode($detailedProduct->colors)) == 1) style="display:none;" @endif>
                                        <div class="col-2">
                            ...
                            <div class="d-table width-100 mt-3">
                                <div class="d-table-cell">
                                    ...
                                    @else
                                        <button type="button" class="btn btn-styled btn-base-3 btn-icon-left strong-700"
                                            disabled>
                            ...
                            @if ($refund_request_addon != null && $refund_request_addon->activated == 1 && $detailedProduct->refundable)
                                <div class="row no-gutters mt-3">
                                    ...
                                    <div class="col-10">
                                        ...
                                        @endphp
                                        <a href="{{ url('/', [$rpSlug]) }}" target="_blank">
                                            @if ($refund_sticker != null && $refund_sticker->value != null) <img src="{{ uploaded_asset($refund_sticker->value) }}" height="36"> @else <img src="{{ my_asset('frontend/images/refund-sticker.jpg') }}" height="36"> @endif</a>
                                        <a href="{{ url('/', [$rpSlug]) }}" class="ml-2" target="_blank">View
                                            Policy</a>
                                    </div>
                                </div>
                            @endif
    ...
    <section class="gry-bg">
        <div class="container">
                    ...
                    <div class="seller-info-box mb-3">
                        <div class="sold-by position-relative">
                            ...
                            @if ($detailedProduct->added_by == 'seller' && \App\BusinessSetting::where('type', 'vendor_system_activation')->first()->value == 1)
                                <a href="{{ route('shop.visit', $detailedProduct->user->shop->slug) }}"
                                    class="name d-block">{{ $detailedProduct->user->shop->{'name_' . locale()} }}
                                    @if ($detailedProduct->user->seller->verification_status == 1)
                                ...
                                <div class="location">{{ $detailedProduct->user->shop->{'address_' . locale()} }}
                                </div>
                            @else
                                {{ \App\GeneralSetting::first()->{'site_name_' . locale()} }}
                            @endif
                            @php
                                $total = 0;
                                $rating = 0;
                            ...
                            <div class="rating text-center d-block">
                                ...
                                <span class="rating-count d-block ml-0">({{ $total }}
                        ...
                        <div class="row no-gutters align-items-center">
                            @if ($detailedProduct->added_by == 'seller')
                                <div class="col">
                                    <a href="{{ route('shop.visit', $detailedProduct->user->shop->slug) }}"
                                        class="d-block store-btn">{{ translate('Visit Store') }}</a>
                                </div>
                                <div class="col">
                                    <ul class="social-media social-media--style-1-v4 text-center">
                                        <li>
                                            <a href="{{ $detailedProduct->user->shop->facebook }}"
                                                class="facebook" target="_blank" data-toggle="tooltip"
                                        ...
                                        <li>
                                            <a href="{{ $detailedProduct->user->shop->twitter }}"
                                                class="twitter" target="_blank" data-toggle="tooltip"
                                        ...
                                        <li>
                                            <a href="{{ $detailedProduct->user->shop->youtube }}"
                                                class="youtube" target="_blank" data-toggle="tooltip"
            ...
            \App\Product::select(['*', 'name_' . locale() . ' as name', 'slug_' . locale() . ' as slug'])->where('user_id', $detailedProduct->user_id)->orderBy('num_of_sale', 'desc'),
        )->limit(6)->get()
        as $key => $top_product)
                            ...
                            <ul class="nav nav-tabs justify-content-center sticky-top bg-white">
                                ...
                                <li class="nav-item">
                                    <a href="#tab_default_4" data-toggle="tab"
                                        class="nav-link text-uppercase strong-600">{{ translate('Reviews') }}</a>
                                </li>
                            </ul>
                                        ...
                                        <div class="row">
                                            <div class="col-md-12">
                                                <div class="mw-100 overflow--hidden aiz-product-description">
                                                    <?php echo $detailedProduct->{'description_' . locale()}; ?>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <div class="tab-pane" id="tab_default_2">
                                    <div class="fluid-paragraph py-2">
                                        ...
                                        <div class="embed-responsive embed-responsive-16by9 mb-5">
                                            @if ($detailedProduct->video_provider == 'youtube' && $detailedProduct->video_link != null && strpos($detailedProduct->video_link, '=') !== false)
                                                <iframe class="embed-responsive-item"
                                                    src="https://www.youtube.com/embed/{{ explode('=', $detailedProduct->video_link)[1] }}"></iframe>
                                            @elseif ($detailedProduct->video_provider == 'dailymotion' && $detailedProduct->video_link != null && strpos($detailedProduct->video_link, '=') !== false)
                                                <iframe class="embed-responsive-item"
                                                    src="https://www.dailymotion.com/embed/video/{{ explode('video/', $detailedProduct->video_link)[1] }}"></iframe>
                                            @elseif ($detailedProduct->video_provider == 'vimeo' && $detailedProduct->video_link != null && strpos($detailedProduct->video_link, '=') !== false)
                                                <iframe
                                ...
                                <div class="tab-pane" id="tab_default_3">
                                    <div class="py-2 px-4">
                                        <div class="row">
                                            <div class="col-md-12">
                                                <a
                                                    href="{{ uploaded_asset($detailedProduct->pdf) }}">{{ translate('Download') }}</a>
                                            </div>
                                        </div>
    ...
    \App\Review::where('user_id', Auth::user()->id)->where('product_id', $detailedProduct->id)->first() == null)
            ...
            <div class="my-4 bg-white p-3">
                ...
                <div class="caorusel-box arrow-round gutters-5">
                    <div class="slick-carousel" data-slick-items="3" data-slick-xl-items="2" data-slick-lg-items="3"
                        data-slick-md-items="2" data-slick-sm-items="1" data-slick-xs-items="1" data-slick-rows="2">
                        @php
                            $this->category_ids = array_column($detailedProduct->subsubcategoryMany->toArray(), 'id');
                            // dd($this->category_ids);
                            $products = \App\Product::where('products.id', '!=', $detailedProduct->id)
                                ->join('product_sub_sub_categories', function ($join) {
                                    $join->on('products.id', '=', 'product_sub_sub_categories.product_id')->whereIn('product_sub_sub_categories.subsubcategory_id', $this->category_ids);
                                })
                                ->select(['products.*', 'products.name_' . locale() . ' as name', 'products.slug_' . locale() . ' as slug']);
                        @endphp
                        @foreach (filter_products($products)->limit(10)->get()
            ...
            <div class="my-4 bg-white p-3">
                ...
                <div class="row">
                    <div class="col-md-12">
                        <div class="mw-100 overflow--hidden aiz-product-description">
                            <?php
                            if ($detailedProduct->hashtag_ids != null) {
                                $tags = explode(',', $detailedProduct->hashtag_ids);
                            } else {
                                $tags = [];
                            }
                            ?>
                            @if (!empty($tags))
                                @foreach ($tags as $tag)
                                    ...
                                    $hashtag = \App\Tags::select(['*', 'name_' . $language . ' as name'])
                                        ->where('id', $tag)
                                        ->first();
                                    ?>
                                    @if ($hashtag->name)
                                        <a href="{{ route('suggestion.search', $hashtag->name) }}"><span>
                                                {{ $hashtag->name }}, </span></a>
                                    @endif
                                @endforeach
                            @endif
