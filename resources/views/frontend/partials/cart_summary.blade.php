<div class="card sticky-top">
    <div class="card-title py-3">
        <div class="row align-items-center">
            ...
            <div class="col-6 text-right">
                <span class="badge badge-md badge-success">{{ count(Session::get('cart')) }}
    ...
    @foreach (Session::get('cart') as $key => $cartItem)
        @php
        if(isset($cartItem['shipping_date'])){
    ...
    @if (isset($shipping_date))
        <div class="card-title py-3">
            <div class="row align-items-center">
                <div class="col-6">
                    <h6 class="heading">
                        <span>{{ translate('Shipping date') }}</span>
                    </h6>
                </div>

                <div class="col-6 text-right">
                    <span class="text-italic">{{ $shipping_date }}</span>
                </div>
            </div>
        </div>
    @endif
        ...
        <table class="table-cart table-cart-review">
            ...
            <tbody>
                ...
                @foreach (Session::get('cart') as $key => $cartItem)
                    ...
                    $product_name_with_choice = $product->{'name_'.locale()}.' - '.$cartItem['variant'];
                    }
                    @endphp
                    <tr class="cart_item">
            ...
            <tfoot>
                ...
                @if (Session::has('wallet_discount'))
                    <tr class="cart-shipping">
                        ...
                        <td class="text-right">
                            <span class="text-italic">{{ single_price(Session::get('wallet_discount')) }}</span>
                        </td>
                    </tr>
                @endif
