   ...
   <form class="form-horizontal" action="{{ route('home_categories.update', $homeCategory->id) }}" method="POST"
               ...
               <select class="select2 form-control aiz-selectpicker" name="category_id" data-toggle="select2"
                   ...
                   @foreach (\App\Category::where('level',0)->where('published',true)->select(['*', 'name_' . locale() . ' as name'])->get() as $category)
                       <option value="{{ $category->id }}" @php
                           if ($homeCategory->category_id == $category->id) {
                               echo 'selected';
                           }
                       @endphp>{{ __($category->name) }}</option>
                   @endforeach
               </select>
           </div>
       </div>
